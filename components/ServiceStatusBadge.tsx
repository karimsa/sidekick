...
export const ServiceStatusBadge: React.FC<{
	...
	switch (status) {
		...
		case HealthStatus.zombie:
			return (
				<Tooltip
					content={'The dev server is running, but is not owned by sidekick.'}
					placement={'right'}
				>
		...
		case HealthStatus.stale:
			return (
				<Tooltip
					content={
						'No dev server is running, but the compiled version of this package is now out-of-date.'
					}
					placement={'right'}
				>
		...
		case HealthStatus.paused:
			return (
				<Tooltip
					content={'You have paused the dev servers.'}
					placement={'right'}
				>
					<span className={'text-orange-700'}>{status}</span>
				</Tooltip>
